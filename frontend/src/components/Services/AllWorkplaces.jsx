...
import { getWorkplaces, deleteWorkplace } from "../../redux/actions/workplace";
...
const AllWorkplaces = () => {
  ...
  const handlePreview = (params) => {
    ...
    const filteredWorkplace = workplace.filter(
      (workplace) => workplace.id === params.id
    );
    setSource(filteredWorkplace);
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => {
        return (
          <>
            <Link to={`/workplaces/workplace-update/${params.id}`}>
              <Button>
    ...
    <>
      {workplaceLoading ? (
        ...
        <div className="w-full mx-8 bg-white">
          <h5 className="text-[30px] font-Poppins text-center">Workplaces</h5>
          <div className="w-full bg-white flex justify-end">
            <Link to={`/workplaces/workplace-post`}>
